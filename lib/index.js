  ...
  getTicket(callback) {
    ...
    if (ticket && ticket.exp > hawk.utils.now()) {
      ...
      requestTicket(function (response) {
        ...
        document.cookie = "bpc_auid=" + response.user + ";expires=" + expiresDate
        + ";domain=." + document.domain + ";path=/";
        document.cookie = "bpc_auti=" + window.btoa(JSON.stringify(response)) + ";expires=" + expiresDate
        + ";domain=." + document.domain + ";path=/";
        callback(response);
      });
    }
...
function requestAuid(callback){

  const url = _bpc_url.concat('/ticket/anonymous');
  var xhr = new XMLHttpRequest();
  xhr.open('GET', url);
  xhr.withCredentials = true;
  xhr.onload = function() {
